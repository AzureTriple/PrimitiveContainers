...
/** A data structure which maps {@code short}s to {@code long}s. */
public record ShortToLongMap(ShortSet keys,LongList values) implements Cloneable
{
    public ShortToLongMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ShortToLongMap(final ShortToLongMap other) {this(new ShortSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final short key,final long value) {values.insert(keys.add(key),value);}
     ...
     */
    public boolean add(final short key,final long value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public long get(final short key) {return values.get(~keys.data().find(key));}
    ...
    public long extract(final short key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ShortToLongMap clone() {return new ShortToLongMap(this);}
}