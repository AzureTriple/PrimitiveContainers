...
/** A data structure which maps {@code short}s to {@code double}s. */
public record ShortToDoubleMap(ShortSet keys,DoubleList values) implements Cloneable
{
    public ShortToDoubleMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ShortToDoubleMap(final ShortToDoubleMap other) {this(new ShortSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final short key,final double value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final short key,final double value)
    {
     ...
     */
    public boolean add(final short key,final double value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public double get(final short key) {return values.get(~keys.data().find(key));}
    ...
    public double extract(final short key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ShortToDoubleMap clone() {return new ShortToDoubleMap(this);}
}