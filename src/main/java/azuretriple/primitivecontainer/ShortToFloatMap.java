...
/** A data structure which maps {@code short}s to {@code float}s. */
public record ShortToFloatMap(ShortSet keys,FloatList values) implements Cloneable
{
    public ShortToFloatMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ShortToFloatMap(final ShortToFloatMap other) {this(new ShortSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final short key,final float value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final short key,final float value)
    {
     ...
     */
    public boolean add(final short key,final float value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public float get(final short key) {return values.get(~keys.data().find(key));}
    ...
    public float extract(final short key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ShortToFloatMap clone() {return new ShortToFloatMap(this);}
}