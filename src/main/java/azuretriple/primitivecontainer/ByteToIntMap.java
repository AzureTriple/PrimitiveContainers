...
/** A data structure which maps {@code byte}s to {@code int}s. */
public record ByteToIntMap(ByteSet keys,IntList values) implements Cloneable
{
    public ByteToIntMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ByteToIntMap(final ByteToIntMap other) {this(new ByteSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final byte key,final int value) {values.insert(keys.add(key),value);}
     ...
     */
    public boolean add(final byte key,final int value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public int get(final byte key) {return values.get(~keys.data().find(key));}
    ...
    public int extract(final byte key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ByteToIntMap clone() {return new ByteToIntMap(this);}
}