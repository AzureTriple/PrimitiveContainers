...
/** A data structure which maps {@code int}s to {@code short}s. */
public record IntToShortMap(IntSet keys,ShortList values) implements Cloneable
{
    public IntToShortMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public IntToShortMap(final IntToShortMap other) {this(new IntSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final int key,final short value) {values.insert(keys.add(key),value);}
     ...
     */
    public boolean add(final int key,final short value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public short get(final int key) {return values.get(~keys.data().find(key));}
    ...
    public short extract(final int key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public IntToShortMap clone() {return new IntToShortMap(this);}
}