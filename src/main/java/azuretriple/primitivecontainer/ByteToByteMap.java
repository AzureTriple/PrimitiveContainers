...
/** A data structure which maps {@code byte}s to {@code byte}s. */
public record ByteToByteMap(ByteSet keys,ByteList values) implements Cloneable
{
    public ByteToByteMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ByteToByteMap(final ByteToByteMap other) {this(new ByteSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final byte key,final byte value) {values.insert(keys.add(key),value);}
    ...
    public byte get(final byte key) {return values.get(~keys.data().find(key));}
    ...
    public byte extract(final byte key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ByteToByteMap clone() {return new ByteToByteMap(this);}
}