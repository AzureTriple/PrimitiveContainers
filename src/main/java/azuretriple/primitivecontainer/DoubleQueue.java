...
/** A queue containing {@code double}s. */
public interface DoubleQueue extends DoubleContainer
{
    /** @return {@code true} iff this queue contains the same data as the argument. */
    default boolean equals(final DoubleQueue other)
    {
    ...
    default double pop() {final double o = data()[((Queue)this).start]; delete(); return o;}
    /** @return The value at the front (i.e. least recently pushed value) of the queue. */
    default double front() {assert !empty(); return data()[((Queue)this).start];}
    /** @return The value at the back (i.e. most recently pushed value) of the queue. */
    default double back() {assert !empty(); return data()[((Queue)this).end-1];}
    
    @Override DoubleQueue clone();
}