...
/** A data structure which maps {@code byte}s to {@code short}s. */
public record ByteToShortMap(ByteSet keys,ShortList values) implements Cloneable
{
    public ByteToShortMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ByteToShortMap(final ByteToShortMap other) {this(new ByteSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final byte key,final short value) {values.insert(keys.add(key),value);}
     ...
     */
    public boolean add(final byte key,final short value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public short get(final byte key) {return values.get(~keys.data().find(key));}
    ...
    public short extract(final byte key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ByteToShortMap clone() {return new ByteToShortMap(this);}
}