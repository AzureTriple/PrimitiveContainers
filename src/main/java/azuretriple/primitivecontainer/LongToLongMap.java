...
/** A data structure which maps {@code long}s to {@code long}s. */
public record LongToLongMap(LongSet keys,LongList values) implements Cloneable
{
    public LongToLongMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public LongToLongMap(final LongToLongMap other) {this(new LongSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final long key,final long value) {values.insert(keys.add(key),value);}
    ...
    public long get(final long key) {return values.get(~keys.data().find(key));}
    ...
    public long extract(final long key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public LongToLongMap clone() {return new LongToLongMap(this);}
}