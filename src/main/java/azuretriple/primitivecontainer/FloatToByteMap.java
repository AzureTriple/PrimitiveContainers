...
/** A data structure which maps {@code float}s to {@code byte}s. */
public record FloatToByteMap(FloatSet keys,ByteList values) implements Cloneable
{
    public FloatToByteMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public FloatToByteMap(final FloatToByteMap other) {this(new FloatSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final float key,final byte value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final float key,final byte value)
    {
     ...
     */
    public boolean add(final float key,final byte value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public byte get(final float key) {return values.get(~keys.data().find(key));}
    ...
    public byte extract(final float key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public FloatToByteMap clone() {return new FloatToByteMap(this);}
}