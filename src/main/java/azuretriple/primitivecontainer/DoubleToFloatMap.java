...
/** A data structure which maps {@code double}s to {@code float}s. */
public record DoubleToFloatMap(DoubleSet keys,FloatList values) implements Cloneable
{
    public DoubleToFloatMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public DoubleToFloatMap(final DoubleToFloatMap other) {this(new DoubleSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final double key,final float value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final double key,final float value)
    {
     ...
     */
    public boolean add(final double key,final float value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public float get(final double key) {return values.get(~keys.data().find(key));}
    ...
    public float extract(final double key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public DoubleToFloatMap clone() {return new DoubleToFloatMap(this);}
}