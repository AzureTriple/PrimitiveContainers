...
/** A data structure which maps {@code byte}s to {@code long}s. */
public record ByteToLongMap(ByteSet keys,LongList values) implements Cloneable
{
    public ByteToLongMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ByteToLongMap(final ByteToLongMap other) {this(new ByteSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final byte key,final long value) {values.insert(keys.add(key),value);}
     ...
     */
    public boolean add(final byte key,final long value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public long get(final byte key) {return values.get(~keys.data().find(key));}
    ...
    public long extract(final byte key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ByteToLongMap clone() {return new ByteToLongMap(this);}
}