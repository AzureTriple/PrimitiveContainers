...
/** A data structure which maps {@code double}s to {@code int}s. */
public record DoubleToIntMap(DoubleSet keys,IntList values) implements Cloneable
{
    public DoubleToIntMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public DoubleToIntMap(final DoubleToIntMap other) {this(new DoubleSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final double key,final int value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final double key,final int value)
    {
     ...
     */
    public boolean add(final double key,final int value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public int get(final double key) {return values.get(~keys.data().find(key));}
    ...
    public int extract(final double key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public DoubleToIntMap clone() {return new DoubleToIntMap(this);}
}