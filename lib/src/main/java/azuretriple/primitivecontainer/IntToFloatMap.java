...
/** A data structure which maps {@code int}s to {@code float}s. */
public record IntToFloatMap(IntSet keys,FloatList values) implements Cloneable
{
    public IntToFloatMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public IntToFloatMap(final IntToFloatMap other) {this(new IntSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final int key,final float value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final int key,final float value)
    {
     ...
     */
    public boolean add(final int key,final float value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public float get(final int key) {return values.get(~keys.data().find(key));}
    ...
    public float extract(final int key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public IntToFloatMap clone() {return new IntToFloatMap(this);}
}