...
/** A data structure which maps {@code short}s to {@code int}s. */
public record ShortToIntMap(ShortSet keys,IntList values) implements Cloneable
{
    public ShortToIntMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public ShortToIntMap(final ShortToIntMap other) {this(new ShortSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final short key,final int value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final short key,final int value)
    {
     ...
     */
    public boolean add(final short key,final int value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public int get(final short key) {return values.get(~keys.data().find(key));}
    ...
    public int extract(final short key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public ShortToIntMap clone() {return new ShortToIntMap(this);}
}