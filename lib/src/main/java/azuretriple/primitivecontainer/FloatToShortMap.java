...
/** A data structure which maps {@code float}s to {@code short}s. */
public record FloatToShortMap(FloatSet keys,ShortList values) implements Cloneable
{
    public FloatToShortMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public FloatToShortMap(final FloatToShortMap other) {this(new FloatSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final float key,final short value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final float key,final short value)
    {
     ...
     */
    public boolean add(final float key,final short value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public short get(final float key) {return values.get(~keys.data().find(key));}
    ...
    public short extract(final float key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public FloatToShortMap clone() {return new FloatToShortMap(this);}
}