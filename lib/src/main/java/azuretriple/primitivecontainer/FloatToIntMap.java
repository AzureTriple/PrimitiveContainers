...
/** A data structure which maps {@code float}s to {@code int}s. */
public record FloatToIntMap(FloatSet keys,IntList values) implements Cloneable
{
    public FloatToIntMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public FloatToIntMap(final FloatToIntMap other) {this(new FloatSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final float key,final int value) {values.insert(keys.add(key),value);}
    /** Maps the key to the value, overwriting the value at the previous location. */
    public void put(final float key,final int value)
    {
     ...
     */
    public boolean add(final float key,final int value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public int get(final float key) {return values.get(~keys.data().find(key));}
    ...
    public int extract(final float key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public FloatToIntMap clone() {return new FloatToIntMap(this);}
}