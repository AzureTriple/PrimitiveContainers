...
public interface ShortList extends ShortContainer
{
    ...
    default void set(final int location,final short v)
    {
        // Could throw out-of-bounds, but that would mean a branch on a potential hot path.
        assert location < ((List)this).size;
    ...
    default void add(final short[] v) {((List)this).add(v);}
    /**
     * Appends the values in the argument to the end of the list.
     *
     ...
     */
    default void add(final short[] v,final int start,final int length) {((List)this).add(v,start,length);}
    /** Appends the argument to the end of the list. */
    default void add(final short v) {((List)this).addLogic(); data()[((List)this).size++] = v;}
    /** Inserts the specified values immediately before the specified location. */
    default void insert(final int location,final short[] v) {((List)this).insert(location,v);}
    /**
     * Inserts the specified values immediately before the specified location.
     *
     * @param start Starting position in the array.
     * @param length Number of elements to copy.
     */
    default void insert(final int location,final short[] v,final int start,final int length)
    {
        ((List)this).insert(location,v,start,length);
    }
    /** Inserts the specified value immediately before the specified location. */
    default void insert(final int location,final short v) {((List)this).insertLogic(location); data()[location] = v;}
