...
/** A data structure which maps {@code int}s to {@code int}s. */
public record IntToIntMap(IntSet keys,IntList values) implements Cloneable
{
    public IntToIntMap {assert keys.data().list.size() == values.size();}
    /** Creates a copy of the argument. */
    @SuppressWarnings("CopyConstructorMissesField")
    public IntToIntMap(final IntToIntMap other) {this(new IntSet(other.keys),other.values.clone());}

    /** Inserts a key value pair. This method does not check for duplicate keys. */
    public void insert(final int key,final int value) {values.insert(keys.add(key),value);}
     ...
     */
    public boolean add(final int key,final int value)
    {
        final int i = keys.data().find(key);
        if(i < 0)
        {
    ...
    public int get(final int key) {return values.get(~keys.data().find(key));}
    ...
    public int extract(final int key) {return values.extract(keys.remove(key));}

    @SuppressWarnings("MethodDoesntCallSuperMethod")
    @Override public IntToIntMap clone() {return new IntToIntMap(this);}
}